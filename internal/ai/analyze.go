...
合规输出示例（敏感图）：图片被判定为敏感/违规内容时，keywords 与 description 依然只描述安全可见层面。例如一张性感/擦边人像照：keywords 可用 ["人像摄影", "室内写真", "光影人像照片"]；description 聚焦灯光、色调、构图与拍摄风格，绝不出现"性感""裸露""私密"等词；classification 各项仍按真实判定填写，risk_reason 如"人物着装暴露，存在擦边暗示"。再例如一张血腥暴力图：keywords 用 ["新闻纪实摄影", "现场摄影作品"]，description 只谈画面结构与色调，不描述任何血腥细节。
